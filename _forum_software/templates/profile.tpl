...
<div class="p_profile">
	...
	<div class="p_alias p_alias_2">
		<?if($vars->profile_data['p_status']['details'] || $vars->profile_init->profile_self || $vars->moderator):?>
			...
			<div style="height: 15px; border-bottom: 1px solid #ECF1EF; padding: 2.5px 0;">
				<?=($vars->profile_data['online']) ? ' <span style="color: green;">Currently online</span>' : ' <span class="control">Currently offline, Last online '.((!empty($vars->profile_data['last_online']['unix'])) ? $vars->profile_data['last_online']['unix'].' '.$vars->profile_data['last_online']['period'].' ago' : 'Never'.'</span>')?>
			</div>
		<?else:
...
<ul class="p_profile_header">
	...
	<li><a href="<?=$globals->_SERVER['PHP_SELF']?>?action=misc&amp;status=cpanel&amp;cmd=user">Usergroups &raquo;</a></li>
