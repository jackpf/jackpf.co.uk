			...
			foreach($type as $value3)
			{
				...
				if(in_array($value3, array('restricted', 'hidden', 'private', 'closed', 'locked', 'moderated')) && $restriction_type == $value3)
				{
					...
					else
					{
						...
						$accept = (in_array($alias_init->alias, $restriction) || in_array('*', $restriction) || in_array('user:'.$alias_init->alias_mod($alias_init->alias, 'lite:Name'), $restriction) || in_array('user:'.$alias_init->userlevel, $restriction)) ? false : true;
						//negative match
						$accept = (in_array('-'.$alias_init->alias, $restriction) || in_array('-user:'.$alias_init->alias_mod($alias_init->alias, 'lite:Name'), $restriction) || in_array('-user:'.$alias_init->userlevel, $restriction)) ? true : $accept;
...
function child_forum($id, $include_forum = false)
{
	...
	$sql = $db->query("SELECT `F`.`ID`, COUNT(`P`.`ID`) AS `PostCount`, SUM(`P`.`Stats`) AS `Stats` ".((isset($alias_init->alias)) ? ", (SELECT SUM(`D`.`PostCount`) FROM `{$db->tb->Forum_Data}` `D` WHERE `D`.`Alias`='".val::post($alias_init->alias)."' AND `F`.`ID`=`D`.`Forum`) AS `Forum_Data_PostCount`" : null)."
	FROM `{$db->tb->Forum}` `F`
	...
	$children = array('Stats' => 0, 'PostCount' => 0, 'ChildrenCount' => 0, 'ChildUnread' => false);
...
if($status == 'forum')
{
	...
	while($fetch = $db->fetch_array($sql))
	{
		...
		if(restricted($status, 'hidden', false))
		{
			...
			if($type != 'forum')
			{
				$subject = 'Hidden/Pending Moderation';
			}
			else
			{
				continue;
...
else if($status == 'thread')
{
	...
	while($fetch = $db->fetch_array($sql))
	{
		...
		if(restricted($status, 'hidden', false))
		{
			$post = 'Hidden/Pending Moderation';
		}
