...
function error_handler($error_code, $error_message, $error_file, $error_line)
{
	...
	if(in_array($error_code, array(E_USER_ERROR, E_USER_NOTICE), true) || (bool) $config_init->get_config('debug_mode'))
	{
		...
		if(strcmp($error_code, E_USER_ERROR) == 0 || strcmp($error_code, E_USER_NOTICE) == 0)
		{
			...
			'title'      => array($config_init->get_config('site_header'), 'error'),
...
class connection
{
	...
	public function __construct()
	{
		...
		'Forum_Reputation'   => 'Forum_Reputation', //from beta...
		'Blog'               => 'Blog',
		'IM'                 => 'IM'
		);
...
class alias
{
	...
	public function alias_mod($alias, $alias_mod = 'full', $forum_mod_local_moderator = false)
	{
		...
		$sql = $db->query("SELECT `User`, `Picture`, `Status` /*for full_forum mod*/ FROM `{$db->tb->Alias}` WHERE `Alias`='".val::post($alias)."';");
		...
		//hack usergroup if user is a local moderator (forum mod)
		...
		if($alias_mod == 'full_forum')
		{
			...
			'posts'        => val::number_format($posts['post_count']),
			'post_count'   => ($permission['details'] == 1) ? true : false
			));
		}
...
class val
{
	...
	{
		return (preg_match('/(.*?)\@(.*?)\.(.*?)/', $email) && checkdnsrr(end(explode('@', $email)), 'MX')) ? $email : null;
	}
...
class profile
{
	...
	{
		/*
		 *profile_status	(0 = normal, 1 = hidden)
...
class template #extends page, profile, form, parse
{
	...
	{
		$template_file = $_SERVER['DOCUMENT_ROOT'].'/templates/'.$template.'.tpl';
		$template_file = (!file_exists($template_file)) ? $template : $template_file;
