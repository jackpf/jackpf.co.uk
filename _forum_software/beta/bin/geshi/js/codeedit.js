...
//---------------------------------------------------------------------------------------------------------
	...
	this.insertMarker = function(init) {
		if(IE && !init) this.insertText('\u0001');
		else if(GK) {
...
//---------------------------------------------------------------------------------------------------------
	...
	this.keyUpHandler = function(e) {
		...
		if(!ignoreKey && !moveKey) {
			...
			this.timer = setTimeout(this.syntaxHilight.bind(this), 0);
		}
	}
...
var languages = {
	...
	php: {
		tags: {
			...
			replace: [ 'ÂµÂµ$1ÂµÂµÂµ' ]
		},
		operators: {
			match: [ /\/\*/g, /\*\//g, /\/\//g, /((&amp;)+|(&lt;)+|(&gt;)+|[\|!=%\*\/\+\-]+)/g, /\u0002/g, /\u0003/g, /\u0004/g, /ÂµÂµ(.+?)ÂµÂµÂµ/g ],
