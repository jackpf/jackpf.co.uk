...
class blog
{
	...
	public function entry_existence($type)
	{
		...
		{
			return trigger_error(sprintf($tpl->lang('blog.VIEW_NOT_EXIST'), $type));
		}
		//check for mutiple entry existence, in case of reference by subject
		else if($db->count_rows($sql) > 1)
		{
			#return trigger_error($tpl->lang('blog.VIEW_MULTIPLE_EXISTENCE');
		}
	}
...
if($status == 'index')
{
	...
	$page->delimiter = '/page=';
	$page->default = 'last';
	$sql = $page->page("SELECT `B`.*, COUNT(`C`.`ID`) AS `CommentCount`
	FROM `{$db->tb->Blog}` `B`
...
{
	$sql = $db->query("SELECT * FROM `{$db->tb->Blog}` WHERE ".((is_numeric($entryid)) ? "`ID`='$entryid'" : "BINARY `Subject`='".$blog->extract_url_subject($entryid)."'")." AND `Type`='entry';");
	...
	{
		trigger_error(sprintf($tpl->lang('blog.VIEW_NO_PERMISSIONS'), 'entry'));
	}
	...
	$page->default = 'last';
	$sql = $page->page("SELECT * FROM `{$db->tb->Blog}` WHERE `ID2`='$entryid' AND `Type`='comment' ORDER BY `ID` ASC;", 5, &$pagination);
...
else if($status == 'comment')
{
	...
	else if(form::submitted())
	{
		...
		{
			trigger_error(sprintf($tpl->lang('blog.POST_NOT_EMPTY'), 'comment'));
		}
		form::unique_check();
		
		$db->query("INSERT INTO `{$db->tb->Blog}` (`ID`, `ID2`, `Type`, `Category`, `Subject`, `Entry`, `Author`, `Unix`, `Edit`, `Status`, `Options`) VALUES (null, '$entryid', 'comment', '$category', '$subject', '$comment', '".val::post($alias_init->alias)."', UNIX_TIMESTAMP(), '0', 'visible', '$options');");
...
{
	$sql = $db->query("SELECT * FROM `{$db->tb->Blog}` WHERE `ID`='$entryid' AND `Type`='comment';");
	$fetch = $db->fetch_array($sql);
	...
	else if(form::submitted())
	{
		...
		if(trim($edit) == null)
		{
			trigger_error(sprintf($tpl->lang('blog.POST_NOT_EMPTY'), 'edit'));
		}
		form::unique_check();
...
else if($status == 'entry')
{
	...
	else if(form::submitted())
	{
		...
		{
			trigger_error(sprintf($tpl->lang('blog.POST_NOT_EMPTY'), 'entry'));
		}
		form::unique_check();
		
		$db->query("INSERT INTO `{$db->tb->Blog}` (`ID`, `ID2`, `Type`, `Category`, `Subject`, `Entry`, `Author`, `Unix`, `Edit`, `Status`, `Options`) VALUES (null, LAST_INSERT_ID(), 'entry', '$category', '$subject', '$entry', '".val::post($alias_init->alias)."', UNIX_TIMESTAMP(), '0', '$visibility', '$options');");
