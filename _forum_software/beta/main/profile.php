...
{
	//parse code
	$profile_data['p_profile_display'] = ($profile_init->alias_init2->userlevel >= alias::user(alias::USR_ADMINISTRATOR)) ? code::parse_code_php($profile_data['p_profile']) : (($profile_init->alias_init2->userlevel >= alias::user(alias::USR_MODERATOR)) ? $profile_data['p_profile'] : val::encode($profile_data['p_profile']));
}
else
{
	//parse code
	$profile_data['p_profile_display'] = parse::parse($profile_data['p_profile'], parse::options_parse_code | parse::options_parse_code_ws | parse::options_parse_smiley);
}
...
$profile_data['forumposts']['rep']                     = val::number_format($profile_data['forumposts']['rep']);
...
if($profile_data['p_status']['profile'] == 2)
{
	//secure private profiles
	if(!$profile_init->profile_self && $profile_init->alias_init->userlevel <= alias::user(alias::USR_MODERATOR))
		$profile_data['p_private_profile'] = true;
}
if($db->count_rows($sql) == 0)
{
...
'moderator'     => ($profile_init->alias_init->userlevel >= alias::user(alias::USR_MODERATOR)) ? true : false
));
...
else if($profile == 'profile' && $profile_init->profile_self)
{
	...
	else if($account == 'forum_subscription')
	{
		$sql = $db->query("SELECT `F`.`Forum` AS `Forum`, `F`.`ID` AS `Thread`, `F`.`Subject` AS `Subject`
		FROM `{$db->tb->Forum_Subscription}` `S`, `{$db->tb->Forum}` `F`
		WHERE `S`.`Alias`='".val::post($profile_init->alias_init2->alias)."' AND `S`.`Thread`=`F`.`ID`
		ORDER BY `F`.`ID`;");
	...
	else if($account == 'forum_reputation')
	{
		$sql = $db->query("SELECT `R`.`Author`, `R`.`Reputation`, `F`.`Forum`, `F`.`Thread`, `F`.`ID`, `F`.`Subject`
		FROM `{$db->tb->Forum_Reputation}` `R`, `{$db->tb->Forum}` `F`
		WHERE `Alias`='".val::post($profile_init->alias_init2->alias)."' AND `R`.`Post`=`F`.`ID`
		ORDER BY `R`.`ID` DESC;");
		...
		while($fetch = $db->fetch_array($sql))
		{
			...
			'forum'      => $fetch['Forum'],
			'thread'     => $fetch['Thread'],
			'author'     => template::profile_link(val::encode($fetch['Author']), 'style="color: '.$alias_init->alias_mod($fetch['Author'], 'lite:Mod').';"'),
			'subject'    => val::encode($fetch['Subject']),
			'reputation' => val::encode($fetch['Reputation'])
			);
		}
...
else if($profile == 'update' && $profile_init->profile_self)
{
	...
	if($update == 'account')
	{
		...
		$query = ($profile_init->alias_init->userlevel > alias::user(alias::USR_MODERATOR)) ? ", `Email`='$email'"/*, .`Alias`='$alias2/*$alias*\/' executed with checks*/."" : null;
		...
		if($email != $email2 && $profile_init->alias_init->userlevel < alias::user(alias::USR_MODERATOR))
		{
	...
	else if($update == 'profile')
	{
		...
		$db->query("UPDATE `{$db->tb->Alias}` SET `Profile`='$p_profile', `Picture`='$picture', `Signature`='$signature', `Status`='$profile_status' WHERE `Alias`='".val::post($profile_init->alias_init2->alias)."';");
	}
...
else if($profile == 'message')
{
	if(!preg_match('/message\_/i', $_SERVER['QUERY_STRING']))
	{
		...
		while($fetch = $db->fetch_array($sql))
		{
			...
			'author' => ($profile_init->alias_init->alias == $author || $profile_init->alias_init->userlevel >= alias::user(alias::USR_MODERATOR)) ? true : false
			);
	...
	else if(isset($_GET['message_send']) && !$profile_init->profile_self)
	{
		...
		if($profile_data['p_status']['message'] == 0 && $profile_init->alias_init->userlevel < alias::user(alias::USR_MODERATOR))
		{
...
else if($profile == 'email')
{
	...
	else if(!$profile_init->profile_self)
	{
		...
		if($profile_data['p_status']['email'] == 0 && $profile_init->alias_init->userlevel < alias::user(alias::USR_MODERATOR))
		{
		...
		else if(form::submitted())
		{
			...
			//recipients
			$recipients = val::array_explode(',', $_POST['recipient']);
