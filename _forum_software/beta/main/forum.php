	...
	foreach($status as $value)
	{
		...
		foreach(preg_split('/(\)|\;|\)\;)/', preg_replace('/\s/', null, $value)) as $value2)
		{
			...
			foreach($type as $value3)
			{
				...
				{
					//default restriction
					$restriction = ($restriction == null) ? alias::user(alias::USR_MODERATOR) : $restriction;
					
					//userlevel-based restriction
					if(is_numeric($restriction))
					{
						$return = ($die === true) ? secure::restrict($restriction) : (($alias_init->userlevel < $restriction) ? true : $return);
					}
					//alias/user based restriction
					else
					{
						//exctract allowed users
						$restriction = val::array_explode(',', $restriction);
						
						//alias/usergroup matches or wildcard
						$accept = (in_array($alias_init->alias, $restriction) || in_array('*', $restriction) || in_array('user:'.$alias_init->alias_mod($alias_init->alias, 'lite:Name'), $restriction) || in_array('user:'.$alias_init->userlevel, $restriction)) ? true : false;
						//negative match
						$accept = (in_array('-'.$alias_init->alias, $restriction) || in_array('-user:'.$alias_init->alias_mod($alias_init->alias, 'lite:Name'), $restriction) || in_array('-user:'.$alias_init->userlevel, $restriction)) ? false : $accept;
...
function check_post()
{
	...
	$sql = $db->query("SELECT COUNT(`ID`) AS `PostCount` FROM `{$db->tb->Forum}` WHERE `Forum`='$forumid' AND `Thread`='$threadid' AND `Type` IN('thread', 'post');");
	$fetch = $db->fetch_array($sql);
	
	$post_count_new = $fetch['PostCount'];
...
class forum_alias_init
{
	...
	public function mod($id = 'MOD')
	{
		...
		if(isset($this->id))
		{
			...
			//check mod
			$mod = val::array_explode(',', $fetch['Mod']);
			...
			if($fetch['Type'] == 'post' && $id == 'MOD')
			{
				...
				if(!array_key_exists($this->id, $this->t_mod))
				{
					...
					//cache
					$this->t_mod[$this->id] = val::array_explode(',', $fetch['Mod']);
				}
			...
			if(in_array($fetch['Type'], array('thread', 'post')) && $id == 'MOD')
			{
				...
				if(!array_key_exists($this->id, $this->f_mod))
				{
					...
					//cache
					$this->f_mod[$this->id] = val::array_explode(',', $fetch['Mod']);
				}
			...
			if(in_array($alias_init->alias, $mod) || in_array('user:'.$alias_init->alias_mod($alias_init->alias, 'lite:Name'), $mod))
			{
				...
				$this->userlevel_origin = alias::user(alias::USR_MODERATOR);
...
function legacy()
{
	...
	{
		$legacy['Search: '.val::encode($_GET['search'])] = val::encode($_SERVER['REQUEST_URI']);
	}
...
function child_forum($id, $include_forum = false)
{
	...
	$sql = $db->query("SELECT `F`.`ID`, (SELECT COUNT(`T`.`ID`) FROM `{$db->tb->Forum}` `T` WHERE `F`.`ID`=`T`.`Forum` AND `T`.`Type`='thread') AS `ThreadCount`, COUNT(`P`.`ID`) AS `PostCount`, SUM(`P`.`Stats`) AS `Stats` ".((isset($alias_init->alias)) ? ", (SELECT SUM(`D`.`PostCount`) FROM `{$db->tb->Forum_Data}` `D` WHERE `D`.`Alias`='".val::post($alias_init->alias)."' AND `F`.`ID`=`D`.`Forum`) AS `Forum_Data_PostCount`" : null)."
	FROM `{$db->tb->Forum}` `F`
	...
	$children = array('Stats' => 0, 'ThreadCount' => 0, 'PostCount' => 0, 'ChildrenCount' => 0, 'ChildUnread' => false);
	
	while($fetch = $db->fetch_array($sql))
	{
		$children['Stats'] += $fetch['Stats'];
		$children['ThreadCount'] += $fetch['ThreadCount'];
		$children['PostCount'] += $fetch['PostCount'];
		if($fetch['PostCount'] > $fetch['Forum_Data_PostCount'])
		{
			$children['ChildUnread'] = true;
		}
		...
		while($fetch2 = $db->fetch_array($sql2))
		{
			...
			$children['ThreadCount'] += $children_children['ThreadCount'];
...
$forum_alias_init = new forum_alias_init;
$forum_alias_init->mod();
...
{
	$tpl->assign_vars(array(
	'forum_announcement' => parse::parse($config_init->get_config('forum_announcement'), parse::options_parse_code | parse::options_parse_code_ws | parse::options_parse_smiley),
...
//post|administrate forum link
else if($status == 'forum' && !isset($forumid) && $alias_init->userlevel >= alias::user(alias::USR_MODERATOR))
{
	$tpl->assign_vars(array(
	'post_link' => true,
	'admin_link' => ($alias_init->userlevel >= alias::user(alias::USR_ADMINISTRATOR)) ? true : false
	));
}
...
if($status == 'forum')
{
	...
	if(!isset($forumid))
	{
		$sql = $db->query("SELECT `F`.*, (SELECT COUNT(`T`.`ID`) FROM `{$db->tb->Forum}` `T` WHERE `F`.`ID`=`T`.`Forum` AND `T`.`Type`='thread') AS `ThreadCount`, COUNT(`P`.`ID`) AS `PostCount`, ".((isset($alias_init->alias)) ? "(SELECT SUM(`D`.`PostCount`) FROM `{$db->tb->Forum_Data}` `D` WHERE `D`.`Alias`='".val::post($alias_init->alias)."' AND `F`.`ID`=`D`.`Forum`) AS `Forum_Data_PostCount`, " : null)."SUM(`P`.`Stats`) AS `Stats`, MAX(`P`.`ID`) AS `LastPost`
		FROM `{$db->tb->Forum}` `F`
		LEFT OUTER JOIN `{$db->tb->Forum}` `P` ON `F`.`ID`=`P`.`Forum` AND `P`.`Type` IN('thread', 'post')
	...
	else if(isset($forumid))
	{
		...
		$search = array(
		'SELECT'    => array(),
		'WHERE'     => ($forumid != 0) ? "WHERE (`F`.`Forum`='$forumid' OR (`F`.`Type`='thread' AND SUBSTRING_INDEX(`F`.`Status`, ':', -1)=2))" : "WHERE `F`.`Forum`=`F`.`Forum`",
		...
		$sql = $page->page("SELECT `F`.*, IF(`F`.`Type`='forum', (SELECT COUNT(`T`.`ID`) FROM `{$db->tb->Forum}` `T` WHERE `F`.`ID`=`T`.`Forum` AND `T`.`Type`='thread'), null) AS `ThreadCount`, COUNT(`P`.`ID`) AS `PostCount`, IF(`F`.`Type`='forum', (SELECT SUM(`D`.`PostCount`) FROM `{$db->tb->Forum_Data}` `D` WHERE `D`.`Alias`='".val::post($alias_init->alias)."' AND `F`.`ID`=`D`.`Forum`), (SELECT `D`.`PostCount` FROM `{$db->tb->Forum_Data}` `D` WHERE `D`.`Alias`='".val::post($alias_init->alias)."' AND `F`.`Forum`=`D`.`Forum` AND `F`.`ID`=`D`.`Thread`)) AS `Forum_Data_PostCount`, IF(`F`.`Type`='forum', SUM(`P`.`Stats`), `F`.`Stats`) AS `Stats`, MAX(`P`.`ID`) AS `LastPost` ".((count($search['SELECT']) > 0) ? ", " : null).implode(", ", $search['SELECT']).
		"FROM `{$db->tb->Forum}` `F`
	...
	while($fetch = $db->fetch_array($sql))
	{
		...
		$stats = $fetch['Stats'];
		$thread_count = $fetch['ThreadCount'];
		...
		if(!isset($forumid) || $type == 'forum')
		{
			...
			$fetch2 = $children = child_forum($id);
			$thread_count += $fetch2['ThreadCount'];
			$post_count += $fetch2['PostCount'];
			$stats += $fetch2['Stats'];
			$children_count += $fetch2['ChildrenCount'];
			...
			if($fetch['Mod'] != null)
			{
				$forum_mod = val::array_explode(',', $fetch['Mod']);
		...
		'forum_children' => (isset($forum_children) && $type == 'forum') ? $forum_children : null,
		'forum_mod'      => (isset($forum_mod) && $type == 'forum') ? $forum_mod : null,
		'date'           => $unix,
		'stats'          => val::number_format($stats),
		'thread_count'   => val::number_format($thread_count),
		...
		'read'           => ((int) $fetch['PostCount'] /*void child board's post count*/ - (int) $children_count /*void child boards*/ <= (int) $fetch['Forum_Data_PostCount'] && !$children['ChildUnread']) ? true : false, //use lastpost method?
...
else if($status == 'thread')
{
	...
	$f_mod = val::array_explode(',', $fetch['Mod']);
	...
	$post_count = $fetch['PostCount'];
	//merge thread mods onto forum mods
	$f_mod += val::array_explode(',', $fetch['Mod']);
	...
	while($fetch = $db->fetch_array($sql))
	{
		...
		//display message for mods/admins for hidden posts
		if(!restricted($status, 'hidden', false) && $status == 'hidden(0.5)' && $alias_init->userlevel >= alias::user(alias::USR_MODERATOR))
		{
			$subject .= ' - (hidden)';
		}
		
		//check mod against posts
		$forum_alias_init->mod($id);
		
		//define permissions
		$permission = array(
		'author'    => (($alias_init->alias == $author || $alias_init->userlevel >= alias::user(alias::USR_MODERATOR)) && !restricted(array($f_status, $status), array('private', 'closed', 'locked'), false)) ? true : false,
		...
		//get rep status
		$sql2 = $db->query("SELECT null FROM `{$db->tb->Forum_Reputation}` WHERE `Author`='".val::post($alias_init->alias)."' /*`Alias`*/ AND `Post`='$id';");
		...
		'rep'         => ($db->count_rows($sql2) == 0 && $author != $alias_init->alias)
		);
	}
	...
	switch(post_type)
	{
		...
		case 'FORUM':
			...
			if(!form::submitted())
			{
				...
				$tpl->assign_vars(array(
				'permissions'     => $permission,
				'parse_extension' => ($alias_init->userlevel >= alias::user(alias::USR_MODERATOR)) ? true : false
				));
		...
		case 'THREAD':
			...
			else if(form::submitted())
			{
				...
				$permission = ($alias_init->userlevel < alias::user(alias::USR_MODERATOR) && (profile::modded_alias($alias_init->alias) || restricted($f_status, 'moderated', false))) ? 'hidden('.alias::user(alias::USR_MODERATOR).')' : $permission;
				if($alias_init->userlevel < alias::user(alias::USR_MODERATOR))
				{
				...
				if($status == 'hidden('.alias::user(alias::USR_MODERATOR).'):0')
				{
		...
		case 'POST':
			...
			if(!form::submitted())
			{
				...
				if(isset($_GET['reply']))
				{
					$reply = val::array_explode(',', $_GET['reply']);
			...
			else if(form::submitted())
			{
				...
				$permission = $_POST['permission'];
				//sticky
				$sticky = ($alias_init->userlevel >= alias::user(alias::USR_MODERATOR)) ? (int) $_POST['sticky'] : 0;
				
				//check for hidden post
				$permission = ($alias_init->userlevel < alias::user(alias::USR_MODERATOR) && (profile::modded_alias($alias_init->alias) || restricted(array($f_status, $t_status), 'moderated', false))) ? 'hidden('.alias::user(alias::USR_MODERATOR).')' : 'open';
				...
				//find return page/AJAX: return to refered page
				if(!isset($_GET['ajax']))
				{
					$sql = $db->query("SELECT COUNT(`ID`) AS `PostCount` FROM `{$db->tb->Forum}` WHERE `Forum`='$forumid' AND `Thread`='$threadid' AND `Type` IN('thread', 'post');");
					$fetch = $db->fetch_array($sql);
					$page = '&page='.ceil($fetch['PostCount'] / 25);
				}
				else
				{
					$page = '&page='.page::get_http_referer('page');
				}
...
else if(in_array($status, array('forum_permission', 'thread_permission', 'post_permission')))
{
	...
	else if(form::submitted())
	{
		...
		$db->query("UPDATE `{$db->tb->Forum}` SET `Author`='$author', `Edit`='$edit', `Status`='$permission', ".(($type == 'forum') ? "`Forum`='$forum'," : null)." `Mod`='$mod' WHERE `ID`='$_id' AND `Type` IN('forum', 'thread', 'post');");
...
else if(in_array($status, array('forum_delete', 'thread_close', 'post_delete')))
{
	...
	//post count for threads/posts
	if(in_array($type, array('thread', 'post')))
	{
		$sql = $db->query("SELECT null FROM `{$db->tb->Forum}` WHERE `Forum`='$forumid' AND `Thread`='$threadid' AND (`Type`='thread' OR `Type`='post');");
	...
	if(!form::submitted())
	{
		...
		//if it's a thread, assign what's being done to it 
		if($type == 'thread')
		{
			if(restricted($f_status, array('closed', 'locked'), false) || restricted($t_status, array('closed', 'locked'), false))
			{
				$thread_type = 'purge';
			}
			else if($posts == 1)
	...
	else if(form::submitted())
	{
		...
		if($type == 'thread' && $posts > 1 && !restricted($t_status, array('closed', 'locked'), false))
		{
			...
			switch($type)
			{
				...
				default:
					...
					else
					{
						...
						//decrement forum data for thread if a post was deleted from it, and it has the current postcount
						if($type == 'post')
						{
							$query = (array) $query;
							$query[] = "UPDATE `{$db->tb->Forum_Data}` SET `PostCount`=`PostCount`-1 WHERE `Thread`='$threadid' AND `PostCount`='$posts';";
						}
					}
...
else if($status == 'thread_move')
{
	...
	$fetch = forum_info();
	$f_status = reset(sticky($fetch['Status']));
	//forum info² (check the forum being moved to)
	if(form::submitted())
	{
		$_forumid = $forumid;
		$forumid = (int) val::post($_POST['thread_move']);
		$fetch = forum_info();
		$f_status2 = reset(sticky($fetch['Status']));
		$forumid = $_forumid;
	}
	//thread info
	$fetch = thread_info();
	...
	//restrictions
	secure::restrict(alias::user(alias::USR_MODERATOR));
	restricted(array($f_status, $f_status2, $status), array('hidden', 'restricted', 'private', 'closed', 'locked'));
