...
if($status == 'register')
{
	...
	else if(form::submitted())
	{
		...
		$subject = $config_init->get_config('email_prefix').'Profile Authentication';
...
else if($status == 'authentication')
{
	...
	$db->query("UPDATE `{$db->tb->Alias}` SET `User`='{$alias_init->user['User']['Name']}' WHERE SHA1(`Email`)='$crypt';");
	...
	switch($help)
	{
		case 'details':
			...
			else if(form::submitted())
			{
				...
				$subject = $config_init->get_config('email_prefix').'Requested Details';
