...
<div class="p_alias p_alias_1">
	<div class="p_alias_1_container">
		<?if(!empty($vars->profile_data['picture'])):
			print '<img class="p_picture" src="'.$vars->profile_data['picture'].'" alt="'.$vars->profile_init->alias_init2->alias.'\'s Picture" />';
		else:
			print (($vars->profile_init->profile_self) ? 'You have' : $vars->profile_init->alias_init2->alias.' has').' no picture.';
		endif;
		if(!empty($vars->profile_data['music'])):
			print '<br /><object classid="clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B" codebase="http://www.apple.com/qtactivex/qtplugin.cab" width="200" height="16">
				<param name="src" value="'.$vars->profile_data['music'].'" />
				<param name="autoplay" value="false" />
				<embed src="'.$vars->profile_data['music'].'" autostart="false" />
			</object>';
		endif?>
	</div>
</div>
...
<div class="p_alias p_alias_2">
	<?if($vars->profile_data['p_status']['details'] || $vars->profile_init->profile_self || $vars->moderator):
		print 'Alias: '.$vars->profile_init->alias_init2->alias.'<br />
		Name: '.$vars->profile_data['name'].(($vars->profile_init->profile_self || $vars->moderator) ? ' (<a href="mailto:'.$vars->profile_data['email'].'">'.$vars->profile_data['email'].'</a>)' : null).'<br />
		Registered: '.$vars->profile_data['register_date'].'<br />
		Online: '.$vars->profile_data['online_time']['unix'].' '.$vars->profile_data['online_time']['period'].'<br />
		Forum Posts: '.$vars->profile_data['forumposts']['post_count'].' ('.$vars->profile_data['forumposts']['post_count_per_day'].' per day)<br />
		Most Active Board: '.((array_filter($vars->profile_data['forumposts']['active_forum']) != null) ? '<a class="f_legacy" href="'.$globals->_SERVER['PHP_SELF'].'?action=forum&amp;forum='.$vars->profile_data['forumposts']['active_forum']['ID'].'">'.$vars->profile_data['forumposts']['active_forum']['subject'].'</a>' : 'N/A').'<br />
		Usergroup: '.((!in_array($vars->profile_data['usergroup'], array('hidden', 'none'))) ? '<a style="color: '.$vars->profile_data['usergroup']['color'].';" href="'.$globals->_SERVER['PHP_SELF'].'?action=misc&amp;status=user&amp;group=user&amp;user='.$vars->profile_data['usergroup']['name'].'">'.$vars->profile_data['usergroup']['name'].'</a>' : ucfirst($vars->profile_data['usergroup']));
		
		if(!$vars->profile_init->profile_self):
			print '<div class="f_header2">'
				.(($vars->profile_data['online']) ? ' <span style="color: green;">Online</span>' : ' <span class="control">Offline<br />Last online: '.((!empty($vars->profile_data['last_online']['unix'])) ? $vars->profile_data['last_online']['unix'].' '.$vars->profile_data['last_online']['period'].' ago' : 'Never'.'</span>')).
			'</div>';
		endif;
	else:
...
<ul class="p_profile_header">
	...
	<li><a href="<?=$globals->_SERVER['PHP_SELF']?>?action=misc&amp;status=user">Usergroups &raquo;</a></li>
...
<?switch($vars->profile_type):
	case 'profile':?>
		<form id="profile" action="<?=$vars->profile_init->uri?>&amp;profile=update&amp;update=profile" method="post" enctype="application/x-www-form-urlencoded"><div>
			...
			<div class="p_profile_left">
				...
				Music<br /><input type="text" name="music" value="<?=$vars->profile_data['music']?>" /><br /><br />
	...
	case 'account':?>
		...
		<br /><br />
		<a class="control" href="<?=$globals->_SERVER['PHP_SELF']?>?action=register&amp;status=unregister">Delete Account</a>
	<?break;
	case 'cpanel':?>
