...
namespace Steam
{
    class Steam
    {
        ...
        int GetProfiles(SteamProfileArray &Profiles) //retrieves profiles from the ini file and stores them in an array
        {
            //store the profiles in an array
			int NumProfiles = 0;
			DWORD bytes = 1;
            for(int i = 1; bytes > 0/*<= MaxProfiles*/; i++)
            {
                char _i[2];
                itoa(i, _i, 10);
            ...
            for(int i = 1; i <= NumProfiles; i++)
            {
                if(strcmp(SteamProfiles[i], "") != 0)
                {
                    ...
                    int ArgSeperator = Profile.find(" ");//,
                    //ArgEnd = Profile.find(" ", ArgSeperator);

                    std::string Username = Profile.substr(0, ArgSeperator);
                    //std::string Password = Profile.substr(ArgSeperator + 1, ArgEnd);
        ...
        virtual void Close()
        {
            //fetch whether to force shutdown steam or not
            char SteamForce[8];
            GetPrivateProfileString("Steam", "Force", "", SteamForce, sizeof(SteamForce), IniFile);

            if(strcmp(SteamForce, "false") == 0)
                Run("-shutdown");
            else
            {
                char ProcessInfo[64];
