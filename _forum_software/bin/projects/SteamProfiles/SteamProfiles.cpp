...
int main(int argc, char *argv[])
{
    ...
    int NumProfiles = Steam.GetProfiles(ProfileNames, true);
        ...
        if(Steam.SetProfile(atoi(ProfileIndex), LoginInfo))
        {
            if(Steam.CanRun())
            {
                if(Steam.IsRunning())
                {
                    ...
                    {
                        Steam.Close((LoadAttempt >= Steam::MaxLoadAttempts));

                        cout << "Closing process" << ((LoadAttempt >= Steam::MaxLoadAttempts) ? " (forced)" : /*" (graceful)"*/"") << "... 0%";
                        for(int i = 1; i <= 100; i++)
                        {
                            Sleep(10);
                            cout << "\b\b" << ((i > 10) ? "\b" : "") << i << "%";
                ...
                if(!Steam.IsRunning())
                {
                    ...
                    cout << "Logged in successfully with profile: " << ProfileNames[atoi(ProfileIndex)] << "." << endl;
                    ...
                    cout << "Unable to close and re-open steam after " << Steam::MaxLoadAttempts << " tries." << endl;
            }
            else
