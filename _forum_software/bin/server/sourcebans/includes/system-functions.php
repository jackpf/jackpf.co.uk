...
function AddTab($title, $url, $desc, $active=false)
{
	...
	$tab_arr[4] = "Protest a ban";		$tab_arr[5] = "Messages";
	$tabs = array();
	$tabs['title'] = $title;
	$tabs['url'] = $url;
	$tabs['desc'] = $desc;
...
function BuildPageTabs()
{
	...
	if($GLOBALS['config']['config.enableprotest']=="1")
		AddTab("Protest a ban", "index.php?p=protest", "Here you can protest your ban. And prove your case as to why you should be unbanned.");	AddTab("Messages", "index.php?p=messages", "Messages left.");		
