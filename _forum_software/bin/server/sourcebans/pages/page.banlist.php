...
if (isset($_GET['a']) && $_GET['a'] == "unban" && isset($_GET['id']))
{
	...
	foreach($bids AS $bid) {
		...
		{
			SendRconSilent(($row['type']==0?"removeid ".(($tempban['modfolder']=='left4dead')?$row['authid_l4d']:$row['authid']):"removeip ".$row['ip']), $tempban['sid']);
		}
		if(((int)$row['now'] - (int)$row['created']) <= 300 && $row['sid'] != "0" && !in_array_dim($row['sid'], $blocked))
			SendRconSilent(($row['type']==0?"removeid ".(($row['modfolder']=='left4dead')?$row['authid_l4d']:$row['authid']):"removeip ".$row['ip']), $row['sid']);

		if($res){
			if(!isset($_GET['bulk']))
				echo "<script>ShowBox('Player Unbanned', '".StripQuotes($row['name'])." (" . ($row['type']==0?(($row['modfolder']=='left4dead')?$row['authid_l4d']:$row['authid']):$row['ip']) . ") has been unbanned from SourceBans.', 'green', 'index.php?p=banlist$pagelink');</script>";
			$log = new CSystemLog("m", "Player Unbanned", "'".StripQuotes($row['name'])."' (" . ($row['type']==0?(($row['modfolder']=='left4dead')?$row['authid_l4d']:$row['authid']):$row['ip']) . ") has been unbanned");
			$ucount++;
		}else{
			if(!isset($_GET['bulk']))
...
else if(isset($_GET['a']) && $_GET['a'] == "delete")
{
	...
	foreach($bids AS $bid) {
		...
		if(empty($steam['RemoveType']))
		{
			foreach($blocked as $tempban)
			{
				SendRconSilent(($row['type']==0?"removeid ".(($tempban['modfolder']=='left4dead')?$steam['authid_l4d']:$steam['authid']):"removeip ".$steam['ip']), $tempban['sid']);
			}
			if(((int)$steam['now'] - (int)$steam['created']) <= 300 && $steam['sid'] != "0" && !in_array_dim($steam['sid'], $blocked))
				SendRconSilent(($row['type']==0?"removeid ".(($steam['modfolder']=='left4dead')?$steam['authid_l4d']:$steam['authid']):"removeip ".$steam['ip']), $steam['sid']);
		}

		if($res){
			if(!isset($_GET['bulk']))
				echo "<script>ShowBox('Ban Deleted', 'The ban for \'".StripQuotes($steam['name'])."\' (".($steam['type']==0?(($steam['modfolder']=='left4dead')?$steam['authid_l4d']:$steam['authid']):$steam['ip']).") has been deleted from SourceBans', 'green', 'index.php?p=banlist$pagelink');</script>";
			$log = new CSystemLog("m", "Ban Deleted", "Ban '".StripQuotes($steam['name'])."' (" . ($steam['type']==0?(($steam['modfolder']=='left4dead')?$steam['authid_l4d']:$steam['authid']):$steam['ip']) . ") has been deleted.");
			$dcount++;
		}else{
			if(!isset($_GET['bulk']))
...
while (!$res->EOF)
{
	...
	if($res->fields['row_type'] == 'D' || $res->fields['row_type'] == 'U' || ($res->fields['ban_length'] && $res->fields['ban_ends'] < time()))
	{
		...
		elseif($res->fields['row_type'] == "U")
			...
			$data['ub_reason'] = ($res->fields['ban_length'] != -60) ? "(Expired)" : "Kicked";
