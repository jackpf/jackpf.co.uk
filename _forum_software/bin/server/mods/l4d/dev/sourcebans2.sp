...
{	
	...
	RegConsoleCmd("sm_message", Message);
	RegAdminCmd("sm_kick", CommandKick, ADMFLAG_KICK, "sm_kick <#userid|name> [reason]");
...
public Action:Message(client, args)
{
	if(GetCmdArgs() < 1)
	{
		ReplyToCommand(client, "%sUsage: sm_message <comment>", Prefix);
		return Plugin_Handled;
	}
	
	decl String:Query[512], String:Comment[128], String:SQL_Comment[128], String:Name[MAX_NAME_LENGTH], String:SQL_Name[MAX_NAME_LENGTH], String:AuthID[64], String:SQL_AuthID[64];
	GetCmdArg(1, Comment, sizeof(Comment));
	if(StrEqual(Comment, ""))
	{
		ReplyToCommand(client, "%sMessage cannot be NULL", Prefix);
		return Plugin_Handled;
	}
	SQL_EscapeString(Database, Comment, SQL_Comment, sizeof(SQL_Comment));
	GetClientName(client, Name, sizeof(Name));
	SQL_EscapeString(Database, Name, SQL_Name, sizeof(SQL_Name));
	GetClientAuthString(client, AuthID, sizeof(AuthID));
	SQL_EscapeString(Database, AuthID, SQL_AuthID, sizeof(SQL_AuthID));
	
	Format(Query, sizeof(Query), "INSERT INTO `%s_Messages` (`ID`, `Client_Name`, `Client_ID`, `Message`) VALUES (NULL, '%s', '%s', '%s');", DatabasePrefix, SQL_Name, SQL_AuthID, SQL_Comment);
	SQL_TQuery(Database, MessageCallback,  Query);
	
	ReplyToCommand(client, "%sMessage left.", Prefix);
	
	return Plugin_Handled;
}
public MessageCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	// If error is not an empty string the query failed
	if(error[0] != '\0')
		LogToFile(logFile, "Message Query Failed: %s", error);
	
	PrintToServer("%sMessage successfully left", Prefix);
}

public Action:CommandKick(client, args)
{
	if(args < 1)
	{
		ReplyToCommand(client, "%sUsage: sm_kick <#userid|name> [reason]", Prefix);
		return Plugin_Handled;
	}
	...
	//SBBanPlayer(client, target, -1, reason);
	decl String:Name[MAX_NAME_LENGTH], String:AuthID[64], String:IP[64], String:Admin_AuthID[64], String:Admin_IP[64];
	GetClientName(target, Name, sizeof(Name));
	GetClientAuthString(target, AuthID, sizeof(AuthID));
	GetClientIP(target, IP, sizeof(IP));
	...
	UTIL_InsertBan(-1, Name, AuthID, IP, reason, Admin_AuthID, Admin_IP, dataPack);
