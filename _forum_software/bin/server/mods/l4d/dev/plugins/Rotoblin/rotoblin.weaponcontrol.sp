...
public _WC_OnEntityCreated(entity, const String:classname[])
{
	for (new i = 0; i < WEAPON_REPLACEMENT_TOTAL; i++)
	{
		...
		DebugPrintToAllEx("OnEntityCreated - Late spawned tier 2. Entity %i, classname \"%s\", new classname \"%s\"", 
			entity, 
			classname, 
			WEAPON_REPLACEMENT_ARRAY[i][WEAPON_REPLACECLASSNAME]);
	}
...
public Action:_WC_ReplaceTier2_Delayed_Timer(Handle:timer, Handle:pack)
{
	...
	{
		DebugPrintToAllEx("Replaced delayed tier 2 weapon; Entity index invalided! Entity %i, classname \"%s\", new classname \"%s\", count %i", entity, classname, newClassname, count);
		return; // Entity is now invalided, skip
	}
 ...
 * rotation.
 *
 * @param entity		Entity index to replace.
 ...
 * @noreturn
 */
static ReplaceTier2_Delayed(entity, const String:classname[], const String:model[], const String:newClassname[], const String:newModel[], count, const Float:time)
{
