...
class img2 extends Steam
{
	...
	private function create_img()
	{
		...
		$background = imagecreatefrompng('resources/images/steam.png');
		imagecopy($img, $background, 0, 0, 0, 0, imagesx($background), imagesy($background));
		...
		imagettftext($img, 16, 0, 4, 18, imagecolorallocate($img, 0, 0, 0), 'resources/fonts/game.ttf', 'Steam');
		imagettftext($img, 12, 0, 60, 18, imagecolorallocate($img, 0, 0, 0), 'resources/fonts/arial.ttf', ' : '.val::str_trim($this->nick, 8));
		
		if(!preg_match('/\<error\>\<\!\[CDATA\[(.*?)\]\]\>\<\/error\>/', $this->profile, &$error))
		{
			imagestring($img, 4, 16, 24, 'Past 2 Weeks: ', imagecolorallocate($img, 255, 255, 255));
			imagestring($img, 3, 128, 24 + 2, val::number_format($this->hours_week * 2).' hrs', imagecolorallocate($img, 112, 147, 219));
			imagestring($img, 4, 16, 38, 'Overall: ', imagecolorallocate($img, 255, 255, 255));
			imagestring($img, 3, 128, 38 + 2, val::number_format($this->hours_total).' hrs', imagecolorallocate($img, 112, 147, 219));
		}
		else
		{
